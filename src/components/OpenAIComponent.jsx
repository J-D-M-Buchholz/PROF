...
const languages = ['Deutsch', 'Englisch', 'Französisch', 'Spanisch', 'Italienisch','Türkisch','Polnisch','Koreanisch','Arabisch','Japanisch','Russisch','Chenesisch'];

const pleaseAnswer = [' bitte antworte auf deutsch.',' please answer in english. ',' merci de répondre en français. ',' por favor responde en español. ',' per favore rispondi in italiano. ',' lütfen türkçe cevaplayın. ',' proszę o odpowiedź po polsku ',' 한국어로 대답해주세요 ',' ارجو الاجابة بالعربية. ',' 日本語で答えてください。 ',' Пожалуйста, ответьте по-русски. ',' 請用中文回答。 '];
...
const handleSubmitChatForm = async (event) => {
  ...
  const newMessages = [...messages, { text: displayQuestion, type: 'question' }];
...
return (
    <div className="container">
      <h1 className="container-header">
        <button type="button" className="exit-button" onClick={handlePopup}>X</button>
        <button type="button" className="language-button" onClick={handleLanguage}>{languages[languageIndex]}</button>
        <img src="sticker/zipper mouth.png" alt="Example Image" className="image" />
        Prof.Dr.GPT
      </h1>
      <div className="container-image">
        <img src="sticker/equation meme.png" alt="Example Image" className="bg-image" />
      </div>
      <form className="form" onSubmit={handleSubmitApiKeyForm}>
        <TooltipSet maxTokens={maxTokens} handleMaxTokensChange={handleMaxTokensChange} temperature={temperature} handleTemperatureChange={handleTemperatureChange} chatOrCode={chatOrCode} handleChatOrCodeChange={handleChatOrCodeChange}>
          <Tooltip>
            <input className="input" type="text" placeholder="Bitte API-Schlüssel eingeben..." value={apiKeyInputValue} onChange={handleApiKeyInputChange} />
          </Tooltip>
        </TooltipSet>
        <input className="submit" type="submit" value="Absenden" />
      </form>
    </div>
 )
}
...
return (
    <div className="container">
      ...
      <button type="button" className="exit-button" onClick={handlePopup}>X</button>
      <button type="button" className="language-button" onClick={handleLanguage}>{languages[languageIndex]}</button>
        <img src={isLoading ? "sticker/hmmm.png":"sticker/sticker.png"} alt="Example Image" className="image" />
        <button className={chatOrCode?"chat":"code"} onClick={handleChatOrCodeChange}>{chatOrCode?"Frage/Antwortmodus":"Codeüberprüfung"}</button>
        Prof.Dr.GPT
      </h1>
      <div className="container-chat">
        {messages.map((message) => (
        <div className={message.type === 'question' ? 'question' : 'answer'}>
        {message.text}
        </div>
        ))}
      ...
      <form className="form" onSubmit={handleSubmitChatForm}>
        <input className="input" type="text" placeholder="Bitte gebe deine Frage ein..." value={question} onChange={handleQuestionChange} />
        <input className="submit" type="submit" value="Absenden" />
      </form>
    </div>
 )
    ...
    <div className="container">
      ...
      <div className="answerCode">{answer}</div>
        <form className="formCode" onSubmit={handleSubmitChatForm}>
          <textarea className="inputCode" placeholder="Bitte füge deinen Code ein, damit ich ihn prüfen kann..." value={question} onChange={handleQuestionChange} />
          <input className="submitCode" type="submit" value="Absenden" />
        </form>
    </div>
)
};
return (
    <div className="popup" onClick={handlePopup}>
      <img src="sticker/hallo.png" alt="Example Image" className="image-popup" />
    </div>
)
};
